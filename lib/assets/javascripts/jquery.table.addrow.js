...
(function($){if(typeof $.fn.btnAddRow== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			...
			$(".autoAdd"+this.seed)
			.live("keyup",function(){
				if((this.nodeName.toString().toLowerCase()=="textarea" && $(this).html()!="") ||
				(this.nodeName.toString().toLowerCase()=="textarea" && $(this).val()!="") ||
				(this.nodeName.toString().toLowerCase()=="select" && $(this).val()!="") ||
				(this.nodeName.toString().toLowerCase()=="input" && $(this).val()!="")) t.addRow();
			});
			this.goLive=true;
		}
		return this
	...
	ExpandableTable.prototype.updateRowCount=function(){
		if(this.displayRowCountTo){
			...
			$("."+this.displayRowCountTo,this.target).each(function(i,v){
				var nn=v.nodeName.toString().toLowerCase();
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			if(this.ignoreClass && this.ignoreClass!=""){
				newRow.find("."+this.ignoreClass).each(function(){
					if(this.nodeName.toString().toLowerCase()=="input" &&
						($(this).attr("type").toLowerCase()=="text"
						|| $(this).attr("type").toLowerCase()=="hidden"
					)) $(this).val("");
					else if(this.nodeName.toString().toLowerCase()=="td") $(this).html(" ");
		...
		this.each(function(){
			...
			if(typeof options.cloneClass=="string" && options.cloneClass!=""){
				if ($(this).closest("table").find("."+options.cloneClass).size()>0){
					...
					tbl=(this.nodeName.toString().toLowerCase()=="table")?$(this):$(this).closest("table");
				}
			}else{
				tbl=(this.nodeName.toString().toLowerCase()=="table")?$(this):$(this).closest("table");
			}
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					if(this.nodeName.toString().toLowerCase()!="table")
						$(this)
